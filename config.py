...
def get_config() -> ml_collections.ConfigDict:
    ...
    config.dataset_configs.pp_train = (
            ...
            ''.join([
                        f'|inception_crop_with_mask((96, 96), 5, 32, (6, 6), inkey=("query{i}", "target_mask", "target_box"), outkey=("query{i}", "query{i}_mask", "query{i}_box"))'
                        for i in range(1, n_queries)]) +
            ''.join(
                [f'|flip_with_mask(inkey=("query{i}", "query{i}_mask"), outkey=("query{i}", "query{i}_mask"))' for i in
                 range(n_queries)]) +
            ...
            '|keep("reference"' + ''.join(
        [f', "query{i}", "query{i}_box", "query{i}_mask"' for i in range(n_queries)]) + ')')
    # For IMAGENET-1K
